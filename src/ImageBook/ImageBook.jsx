...
import profile_pic from './alimadilov.jpg';

class ImageBook extends React.Component {
  constructor(props) {
    super(props);

    const routingStrategy = 'hash';
    ...
    if (isMobile) {
      cv = pageparts.map((part, i) => <Page key={i + 1} pageNumber={i + 1} pageTitle={'Önéletrajz'}>{part}</Page>);
      cvPageNumbers = pageparts.length;
    }
    else {
      cv = <Page key={1} pageNumber={1} pageTitle={'Önéletrajz'}>
        {pageparts.reduce((res, part, i) => {
    ...
    const pages = [
      ...
      // </Page>,
      <Page key={cvPageNumbers + 1} pageNumber={cvPageNumbers + 1} pageTitle={'Tartalomjegyzék'}>
        <ul className='toc'>
          {/* <li><a href={((routingStrategy === 'hash') ? '#/' : '') + '1'} onClick={this.navigateToPage}>Önéletrajz</a></li> */}
          <li><a href={((routingStrategy === 'hash') ? '#/' : '') + String(cvPageNumbers + 2)} onClick={this.navigateToPage}>Tengerparton</a></li>
          <li><a href={((routingStrategy === 'hash') ? '#/' : '') + String(cvPageNumbers + 3)} onClick={this.navigateToPage}>Cserépedények</a></li>
          <li><a href={((routingStrategy === 'hash') ? '#/' : '') + String(cvPageNumbers + 4)} onClick={this.navigateToPage}>Virágok az ablakban</a></li>
          <li><a href={((routingStrategy === 'hash') ? '#/' : '') + String(cvPageNumbers + 5)} onClick={this.navigateToPage}>Átjáró</a></li>
        </ul>
      </Page>,
      <Page key={cvPageNumbers + 2} pageNumber={cvPageNumbers + 2} image={process.env.PUBLIC_URL + '/h608.jpg'} imageCaption={'Tengerparton'}></Page>,
      <Page key={cvPageNumbers + 3} pageNumber={cvPageNumbers + 3} image={process.env.PUBLIC_URL + '/h503.jpg'} imageCaption={'Cserépedények'}></Page>,
      <Page key={cvPageNumbers + 4} pageNumber={cvPageNumbers + 4} image={process.env.PUBLIC_URL + '/h607.jpg'} imageCaption={'Virágok az ablakban'}></Page>,
      <Page key={cvPageNumbers + 5} pageNumber={cvPageNumbers + 5} image={process.env.PUBLIC_URL + '/f1014.jpg'} imageCaption={'Átjáró'}></Page>,
